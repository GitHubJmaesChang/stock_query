...
def daily_query_stock_exchange_information(date, mode):
	try:
		...
		print("daily_query_stock_exchange_information (ERROR): Exception")
		print(e)
		raise Exception

	return(type_1_DataFrame)

def merge_data(stock_num, stock_name, excahnge_volume, s_price, 
				high_price, low_price, e_price, category, currentCat, pd_data):
	try:
		...
		print("merge_data (ERROR): exception")
		print(e)
		raise Exception
...
def daily_information(FilePath, sdate):
	...
	for k in stock_dict:
		try:
			...
			print(e)
			print("daily_information: Append empty record")
			# Append empty record
			stock_num.append(-1)
