...
# The list is based on https://www.sslproxies.org/
def buildProxyList():
	global proxyList
	global urlIdx
	...
	while(keeptrying == True):
		...
		try:
			...
			if(len(proxyList) < 3):
				...
				buildProxyList()
			# A new index required after removing the current proxy entry
			proxy_index = random_proxy()
			proxy = proxyList[proxy_index]
